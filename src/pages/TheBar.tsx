...
import { ArrowLeft, ChevronUp, MessageSquare, Plus, Search, User } from "lucide-react";
      ...
      <section className="min-h-screen pt-24 pb-16 bg-background">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <div className="space-y-4 mb-8">
            {answers.map((a) => (
              <div key={a.id} className={`border rounded-lg p-4 ${a.is_top ? "border-accent bg-accent/5" : "border-border bg-card"}`}>
                {a.is_top && <Badge className="mb-2 bg-accent text-accent-foreground text-xs">Top Answer</Badge>}
                <div className="flex gap-3">
                  <div className="flex flex-col items-center gap-1">
                    <button onClick={() => vote("bar_answers", a.id, a.votes)} className="text-muted-foreground hover:text-accent">
                      <ChevronUp size={16} />
                    </button>
                    <span className="text-sm font-bold text-foreground">{a.votes}</span>
                  </div>
                  <div className="flex-1">
                    <p className="text-sm text-foreground whitespace-pre-wrap">{a.body}</p>
                    <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
                      <User size={12} /> {a.profiles?.display_name || "Anon"} · {timeAgo(a.created_at)}
                    </div>
                  </div>
                </div>
              </div>
