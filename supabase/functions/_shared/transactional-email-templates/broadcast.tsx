...
interface Props { subject?: string; bodyHtml?: string; ctaLabel?: string; ctaUrl?: string; siteUrl?: string }

// SECURITY: bodyHtml is sanitized server-side before being passed in.
// We use dangerouslySetInnerHTML here ONLY because the admin authors this
// in markdown which is converted to safe HTML in send-broadcast.
const Broadcast = ({ subject = 'Update from Locus', bodyHtml = '', ctaLabel, ctaUrl, siteUrl = 'https://locus.legal' }: Props) => (
  <Html lang="en" dir="ltr">
    <Head /><Preview>{subject}</Preview>
    <Body style={main}><Container style={container}>
      <Text style={brand}>Loc<span style={accent}>us</span></Text>
      <Section style={card}>
        <Text style={eyebrow}>Update</Text>
        ...
        <Hr style={hr} />
        <Text style={footer}>You're getting this as a Locus user.</Text>
      </Section>
...
export const template = {
  component: Broadcast,
  subject: (d: Record<string, any>) => d.subject ?? 'Update from Locus',
  displayName: 'Admin broadcast',
  stream: 'broadcasts',
  previewData: { subject: 'New: AI-powered CV analysis', bodyHtml: '<p>Try our new CV analyser today.</p>', ctaLabel: 'Try it', ctaUrl: 'https://locus.legal/cv' },
